 ...
 * XML stanzas to the StanzaHandler. 创建新的会话，销毁会话，并提供接收XML节
 * 
...
public class XmppIoHandler implements IoHandler {
	...
	public void sessionOpened(IoSession session) throws Exception {
		...
		log.info("sessionOpened 打开连接： ==》   " + XML_PARSER + ":[" + parser.toString() + "]");
		session.setAttribute(XML_PARSER, parser);
		// Create a new connection
		Connection connection = new Connection(session);
		session.setAttribute(CONNECTION, connection);
		log.info("sessionOpened 打开连接： ==》   " + CONNECTION + ":[" + connection.toString() + "]");
		session.setAttribute(STANZA_HANDLER, new StanzaHandler(serverName, connection));
		log.info("sessionOpened 打开连接： ==》   " + STANZA_HANDLER + ":["
				+ new StanzaHandler(serverName, connection).toString() + "]");
	}
	...
	public void sessionIdle(IoSession session, IdleStatus status) throws Exception {
		...
		if (log.isDebugEnabled()) {
			log.info("Closing connection that has been idle: " + connection);
		}
		connection.close();
	}
