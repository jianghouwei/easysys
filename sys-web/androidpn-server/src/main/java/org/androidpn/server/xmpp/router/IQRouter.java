...
public class IQRouter {
	...
	private final Log log = LogFactory.getLog(IQRouter.class);
	...
	/**
	 * Routes the IQ packet based on its namespace. "get", "set", "result", or
	 * "error"
	 * 
	 * @param packet
	 *            the packet to route
	...
	public void route(IQ packet) {
		if (packet == null) {
			log.info("路由类型3：》》》》》》》》》》》》》》》》》》》》》" + "IQ -- ID:[null 异常]" );
			throw new NullPointerException();
		}
		log.info("路由类型3：》》》》》》》》》》》》》》》》》》》》》" + "IQ -- ID" +":["+ packet.getID() +"]");
	...
	private void handle(IQ packet) {
		try {
			log.info("路由类型3：》》》》》》》》》》》》》》》》》》》》》请求协议类型[" + "iq:auth;jabber:iq:register ;urn:ietf:params:xml:ns:xmpp-bind; null" +"]");
			Element childElement = packet.getChildElement();
			String namespace = null;
			if (childElement != null) {
				namespace = childElement.getNamespaceURI();
				log.info("路由类型3：》》》》》》》》》》》》》》》》》》》》》" + "IQ--> namespace" +":["+ namespace +"]");
			}
			if (namespace == null) {
				if (packet.getType() != IQ.Type.result && packet.getType() != IQ.Type.error) {
					log.info("v 路由类型3：》》》》》》》》》》》》》》》》》》》》》  Unknown packet " + packet);
				}
			} else {
				//使用自己注册的 handler
				...
				if (handler == null) {
					...
					log.info("路由类型3：》》》》》》》》》》》》》》》》》》》》》" + "IQ--> handler.process(packet)" +":["+ packet +"]");
	...
	private IQHandler getHandler(String namespace) {
		...
		log.info("路由类型3：》》》》》》》》》》》》》》》》》》》》》   自定义handler " + namespace2Handlers.get(namespace));
		if (handler == null) {
			for (IQHandler handlerCandidate : iqHandlers) {
				if (namespace.equalsIgnoreCase(handlerCandidate.getNamespace())) {
					...
					log.info("路由类型3：》》》》》》》》》》》》》》》》》》》》》   注册自定义handler " + handlerCandidate);
					break;
				}
			}
